...
public final class ATMSessionImpl extends AbstractATMSession
{
	...
	public ATMSessionImpl(final ATMCard atmCard, final BankImpl bank)
	{
		super(atmCard, bank);
		date = new Date();
	}
		...
		if (isSessionValid())
		{
			if (amount >= 100 && amount <= 10000 && amount % 100 == 0)
			{
				if (bank.getBalance(atmCard.getAccountHolderId()) > amount)
				{
					...
					this.amount = amount;
					return bank.withdrawAmount(atmCard.getAccountHolderId(), amount);
				}
	...
	{
		bank.addMoney(atmCard.getAccountHolderId(), amount);
	}

	@Override
	public ATMReceipt requestReceipt(final long transactionId)
	{
		return new ATMReceipt(transactionId, bank.requestReceipt(transactionId, amount).getAmount());
	}
